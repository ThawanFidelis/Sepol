class AddAttachmentPictureToEquipaments < ActiveRecord::Migration
  def change
    add_column :equipaments, :picture_file_name, :string
    add_column :equipaments, :picture_content_type, :string
    add_column :equipaments, :picture_file_size, :integer
    add_column :equipaments, :picture_updated_at, :datetime
  end

#  def self.down
#    remove_column :equipaments, :picture_file_name
#    remove_column :equipaments, :picture_content_type
#    remove_column :equipaments, :picture_file_size
#    remove_column :equipaments, :picture_updated_at
#  end
end

